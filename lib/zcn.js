...
const readChunk = (offset, chunkSize, file) => new Promise((res, rej) => {
...
async function bulkUpload(options) {
    ...
    const opts = options.map((obj) => {
        ...
        g[readChunkFuncName] = async (offset, chunkSize) => {
            console.log("bulk_upload: read chunk remotePath:" +
                obj.remotePath +
                " offset:" +
                offset +
                " chunkSize:" +
...
async function setWallet(bls, clientID, sk, pk, mnemonic) {
    if (!bls)
        throw new Error("bls is undefined, on wasm setWallet fn");
    if (!sk)
        throw new Error("secret key is undefined, on wasm setWallet fn");
    if (!pk)
        throw new Error("public key is undefined, on wasm setWallet fn");
    if (bridge.walletId !== clientID) {
        ...
        console.log("inside setWallet- bls:", bls, ",secretKey:", bridge.jsProxy.secretKey, ",publicKey:", bridge.jsProxy.publicKey);
...
async function loadWasm(go) {
    ...
    if (!(WebAssembly === null || WebAssembly === void 0 ? void 0 : WebAssembly.instantiateStreaming)) {
    ...
    setTimeout(() => {
        var _a;
        if (((_a = g.__zcn_wasm__) === null || _a === void 0 ? void 0 : _a.__wasm_initialized__) !== true) {
