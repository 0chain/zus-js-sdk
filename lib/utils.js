...
exports.getMinersAndShardersUtils = exports.submitTransaction = exports.getTransactionResponse = exports.getBalanceUtil = exports.doGetReqToRandomMiner = exports.readBytes = exports.doParallelPostReqToAllMiners = exports.parseJson = exports.getReqFromMiner = exports.plainGet = exports.getDownloadReq = exports.getReq = exports.getReqBlobbers = exports.postReqToBlobber = exports.deleteMethodTo0box = exports.postMethodTo0box = exports.getReferrals = exports.getShareInfo = exports.recoverWalletFromCloud = exports.delReq = exports.putReq = exports.postReq = exports.parseWalletInfo = exports.parseAuthTicket = exports.computeStoragePartDataId = exports.toHex = exports.sleep = exports.shuffleArray = exports.hexStringToByte = exports.byteToHexString = exports.getConsensusedInformationFromSharders = exports.smartContractType = exports.zcnContracts = exports.Endpoints = void 0;
...
exports.zcnContracts = {
    ...
    interestPoolSCAddress: 'cf8d0df9bd8cc637a4ff4e792ffe3686da6220c45f0e1103baa609f3f1751ef4',
    dexMintAddress: '6dba10422e368813802877a85039d3985d96760ed844092319743fb3a76712e0',
};
exports.smartContractType = {
    sharders: exports.zcnContracts.storageSCAddress,
    miners: exports.zcnContracts.minerSCAddress,
};
const configJson = {
    miners: [
 ...
 */
const getConsensusedInformationFromSharders = (sharders, url, params, parser) => {
    return new Promise(function (resolve, reject) {
        const urls = sharders.map((sharder) => sharder + url);
        const promises = urls.map((oneUrl) => (0, exports.getReq)(oneUrl, params));
        ...
        BlueBirdPromise.some(promises, percentage)
            ...
            if (consensusErrorResponse === null) {
                ...
                try {
                    reject(parseConsensusMessage((_a = consensusErrorResponse === null || consensusErrorResponse === void 0 ? void 0 : consensusErrorResponse.response) === null || _a === void 0 ? void 0 : _a.data));
                }
                catch (err) {
                    reject(err);
                }
...
exports.getConsensusedInformationFromSharders = getConsensusedInformationFromSharders;
/**
 ...
 */
const computeStoragePartDataId = (allocationId, path, fileName, partNum) => {
    return sha3.sha3_256(allocationId + ":" + path + ":" + fileName + ":" + partNum);
};
exports.computeStoragePartDataId = computeStoragePartDataId;
/**
 ...
 */
const parseAuthTicket = (authTicket) => {
    var buff = new Buffer(authTicket, "base64");
    var data = buff.toString("ascii");
    return JSON.parse(data);
};
exports.parseAuthTicket = parseAuthTicket;
/**
...
exports.parseWalletInfo = parseWalletInfo;
/**
...
const postReq = (url, data, option) => {
    return axios({
        ...
        transformResponse: function (responseData) {
            return (0, exports.parseJson)(responseData);
        },
    });
};
 ...
 */
const recoverWalletFromCloud = (url, AppIDToken, AppPhoneNumber) => {
    return axios({
        method: "get",
        url: url,
        headers: {
...
exports.recoverWalletFromCloud = recoverWalletFromCloud;
/**
 ...
 */
const getShareInfo = (url, clientSignature, clientId, clientkey) => {
    return axios({
        method: "get",
        url: url,
        headers: {
 ...
 * @returns {Promise} - A Promise representing the POST request.
 */
const postMethodTo0box = (url, data, clientId, publicKey, clientSignature, idToken) => {
    const headers = {
...
exports.postMethodTo0box = postMethodTo0box;
/**
 ...
 * @returns {Promise} - A Promise representing the DELETE request.
 */
const deleteMethodTo0box = (url, data, clientId, publicKey, clientSignature, idToken) => {
    const headers = {
        "X-App-ID-TOKEN": idToken,
        "X-App-Client-ID": clientId,
        "X-App-Client-Key": publicKey,
...
exports.deleteMethodTo0box = deleteMethodTo0box;
/**
 ...
 * @returns {Promise} - A Promise representing the POST request.
 */
const postReqToBlobber = (url, data, params, clientId, publicKey, signature) => {
    const headers = {
        "X-App-Client-ID": clientId,
        "X-App-Client-Key": publicKey,
...
exports.postReqToBlobber = postReqToBlobber;
/**
 ...
 */
const getReqBlobbers = (url, params, clientId) => {
...
exports.getReqBlobbers = getReqBlobbers;
/**
 ...
 */
const getDownloadReq = (url, params) => {
    return axios.get(url, { params });
};
exports.getDownloadReq = getDownloadReq;
/**
 ...
 */
const getReqFromMiner = async (url, params) => {
    try {
...
exports.getReqFromMiner = getReqFromMiner;
/**
 ...
 */
const doParallelPostReqToAllMiners = (miners, url, postData) => {
    return new Promise(function (resolve, reject) {
        const urls = miners.map((miner) => miner + url);
        const promises = urls.map((oneUrl) => (0, exports.postReq)(oneUrl, postData));
        let percentage = Math.ceil((promises.length * consensusPercentage) / 100);
        BlueBirdPromise.some(promises, percentage)
            .then(function (result) {
...
exports.doParallelPostReqToAllMiners = doParallelPostReqToAllMiners;
/**
 ...
 */
const doGetReqToRandomMiner = async (miners, url, getData) => {
    return new Promise(async (resolve, reject) => {
        try {
            let urls = miners.map((miner) => miner + url);
            let shuffledMinerUrl = (0, exports.shuffleArray)([...urls]);
            for (let i = 0; i < shuffledMinerUrl.length; i++) {
                const res = await (0, exports.getReqFromMiner)(shuffledMinerUrl[i], getData);
                if (res.status === 200) {
                    return resolve(res);
                }
            }
...
exports.doGetReqToRandomMiner = doGetReqToRandomMiner;
/**
 ...
 */
const getBalanceUtil = async (clientId, domain) => {
    const { data: minersAndSharders } = await (0, exports.getMinersAndShardersUtils)(domain);
    return new Promise((resolve, reject) => {
        (0, exports.getConsensusedInformationFromSharders)(minersAndSharders.sharders, exports.Endpoints.GET_BALANCE, {
            client_id: clientId,
        })
            .then((res) => {
            resolve(res);
            ...
            .catch((error) => {
            if ((error === null || error === void 0 ? void 0 : error.error) === "value not present") {
                resolve({
                    balance: 0,
                });
            }
 ...
 */
const getTransactionResponse = (data) => {
    const txn = {};
    txn.hash = data.hash;
 ...
 */
const submitTransaction = async (ae, toClientId, val, note, transactionType, domain) => {
    const hashPayload = sha3.sha3_256(note);
    const ts = Math.floor(new Date().getTime() / 1000);
    if (cachedNonce === undefined) {
        //initialize by 0 if there is no nonce from getBalance as well
        try {
            const { nonce } = await (0, exports.getBalanceUtil)(ae.id, domain);
            cachedNonce = nonce !== null && nonce !== void 0 ? nonce : 0;
        }
        catch (err) {
            cachedNonce = 0;
        }
    ...
    const bytehash = (0, exports.hexStringToByte)(hash);
    ...
    return new Promise(async function (resolve, reject) {
        const { data: minersAndSharders } = await (0, exports.getMinersAndShardersUtils)(domain);
        (0, exports.doParallelPostReqToAllMiners)(minersAndSharders.miners, exports.Endpoints.PUT_TRANSACTION, data)
            .then((response) => {
            cachedNonce += 1;
            resolve((0, exports.getTransactionResponse)(response.entity));
        })
            .catch((error) => {
            reject(error);
        });
 ...
 */
const getMinersAndShardersUtils = async (domain) => {
    if (!domain)
        return { error: "domain is required" };
    const { error, data } = await (0, requests_1.basicReqWithDispatch)({
        url: domain.startsWith("http") ? `${domain}/network` : `https://${domain}/dns/network`,
        options: { method: "GET" },
    });
    return { error, data };
};
exports.getMinersAndShardersUtils = getMinersAndShardersUtils;
/**
